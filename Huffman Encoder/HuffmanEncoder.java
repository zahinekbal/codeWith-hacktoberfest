...
public class HuffmanEncoder {
    ...
    // frequency comparison of the characters stored in the two nodes.
	...
	public HuffmanEncoder(String feeder) {
       ...
       for(int i=0;i<feeder.length();i++){
    	   ...
    	   fmap.put(ch, fmap.containsKey(ch)?fmap.get(ch)+1:1); // filling map with the frequnecy of the characters of the string.
       }
       ArrayList<Character> keys=new ArrayList<>(fmap.keySet());
       PriorityQueue<Node> pq=new PriorityQueue<>();
	   
	   // adding the map elements into the priority queue.
	   for(char key:keys){
    	   Node node=new Node();
    	   node.data=key;
	   ...
	   //merging the frequencies of the adjacent nodes. 
       while(pq.size()>1){
    	   Node one=pq.remove();
    	   Node two=pq.remove();
    	   Node merge=new Node();
	...
	// Now traversing and putting the nodes data with 0's and 1's.  
	...
	//Encoding the characters with the codes.
